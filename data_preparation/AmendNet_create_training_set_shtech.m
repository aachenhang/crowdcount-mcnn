...
dataset = 'A';
%dataset = 'B';
dataset_name = ['AmendNet_shanghaitech_part_' dataset '_patches_' num2str(N)];
...
for idx = 1:num_images
    ...
    img_idx = strcat(num2str(i), '_',num2str(0));  
    
    if(idx < num_val)
        imwrite(im, [val_path_img num2str(img_idx) '.jpg']);
        csvwrite([val_path_den num2str(img_idx) '.csv'], im_density);
    else
        imwrite(im, [train_path_img num2str(img_idx) '.jpg']);
        csvwrite([train_path_den num2str(img_idx) '.csv'], im_density);
    end
    %%
    %What this block code do?
    for scale = [3,4,8]
        wn2 = w/scale; hn2 = h/scale;
        wn2 =scale * floor(wn2/scale);
        hn2 =scale * floor(hn2/scale);
       ...
       %%
        for j = 1:N
            ...
            img_idx = strcat(num2str(i), '_',num2str(j), '_',num2str(scale));        
